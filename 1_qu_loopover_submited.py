...
class puzzle(list):
    ...
    def read_matrix(self, matrix):
        """be careful, this function clear original self content and rewrite it with new one"""
        self.m = len(matrix)
        self.n = len(matrix[0])
        self.clear()
    ...
    def left(self, row, repetition=1):
        """base method, rotate certain row of puzzle to the left. number of repetition = repetition
    ...
    def right(self, row, repetition=1):
        """base method, rotate certain row of puzzle to the right. number of repetition = repetition
            log moves to history list"""
        for _ in range(repetition):
    ...
    def up(self, column, repetition=1):
        """base method, rotate certain column of the puzzle upward. number of repetition = repetition
         log moves to history list"""
        for _ in range(repetition):
    ...
    def down(self, column, repetition=1):
        """base method, rotate certain column of the puzzle downward. number of repetition = repetition
                 log moves to history list"""
        for _ in range(repetition):
    ...
    def move_to_rb_corner(self, addr):
        """this function move elements with address - addr to the right bottom corner"""
        self.right(addr[0], self.n - addr[1]-1)
        self.down(self.n-1, self.m - addr[0]-1)
        if addr[0] != self.m - 1:  # we need to fix line where the element was, but not if it is in the down line
            self.right(addr[0], (addr[1]+1) % self.n)

    def solve_vertical_line(self, ref, column=0, rows=0):
        # find elements which need to be placed in certain column in first rows. rows=0 means we move 1 element
    ...
    def solve_last_vertical (self, ref):
        """place top n-2 elements in the left column, and we still need one place to solve bottom line"""
...
# submited to codewars. class code is the same. only main function is different.
...
#     problem.read_matrix(mixed_up_board)  # read input matrices and place it into our class
...
#         if problem.is_solved:  # only solve_last_horisontal can set is_solved to True
#             break
#         else:
...
#             problem.shuffle(5)  # oops, bad lack, but if number of element is even
#                                 # we can try to solve it again lets go
#     return problem.history


# standalone varian with autoproblem creation and shuffling.
