...
# with ordered dict and enumerate and trick with addind last meet index to the dict.
# the is the fastest  variant

def lenght_of_longest_subset(input_objects: Iterable, k: int) -> int:
    """return the length of the subset of input_objects  which contain maximum k different object"""
    length =  0
...
def lenght_of_longest_subset1(input_objects: Iterable, k: int) -> int:
    ...
    for index, obj in enumerate(input_objects):
        ...
        if len(right_most_indexes) <= k:
            ...
            elem_to_delete = min(right_most_indexes, key=right_most_indexes.get)
            index_left = right_most_indexes[elem_to_delete]
            right_most_indexes.pop(elem_to_delete)
    return length
...
def lenght_of_longest_subset5(input_objects, k=1):
    ...
    current_set_unique = set()  # set - hold current set of unique elements in current set
    ...
    for obj in input_objects:
        ...
        else:   # we have  k+1 different elements in current_set, so we need to eject elements from it.
                # will do it from left side one by one until current_set will conatain exaclty k different elements
            while len(set(current_set)) > k:
                current_set.pop(0)
            current_set_unique = set(current_set) # replace current_set_unique with new set of unique elements
